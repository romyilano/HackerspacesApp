namespace Hackerspaces
{
	...
	public class HackerspacesListView : MonoBehaviour
	{
		class ItemData
		{
		...
		public string previousSearchTextString = "";
		
		public ListView ListView;
		public GameObject ItemButtonPrefab;
		public Sprite NoisebridgeIcon;
		public Sprite SudoRoomIcon;
		...
		public void Start()
		{
			...
			// Add some images.
			imageList.Images.Add("Noisebridge", NoisebridgeIcon);
			imageList.Images.Add("SudoRoom", SudoRoomIcon);
		...
		private ListViewItem CreateListViewItem(string imageKey, string shipSpecies, string shipName, string growth)
		{
		...
		private void AddListViewItem(string imageKey, string shipSpecies, string shipName, string growth)
		{
			ListViewItem item = CreateListViewItem(imageKey, shipSpecies, shipName, growth);
		...
		private void RefreshTestData()
		{
			...
			if (this.ListView != null)
			{
				this.ListView.SuspendLayout();
				{
					...
					ShipSpeciesColumn.Text = "Hackerspace";
					this.ListView.Columns.Add(ShipSpeciesColumn);

					ColumnHeader ShipNameColumn = new ColumnHeader();
					ShipNameColumn.Text = "Members";
					this.ListView.Columns.Add(ShipNameColumn);

					ColumnHeader AmountInFleetColumn = new ColumnHeader();
					AmountInFleetColumn.Text = "Level";
					this.ListView.Columns.Add(AmountInFleetColumn);
					
					foreach (Hackerspaces.Simulator.Hackerspace hackerspace in Hackerspaces.Simulator.Instance.hackerspaces )
					{
						string iconString = "";
						if (hackerspace.name == "Noisebridge")
						{
							iconString = "NoisebridgeIcon";
						}
						else if (hackerspace.name == "Sudo Room")
						{
							iconString = "SudoRoomIcon";
						}
						AddListViewItem(iconString, hackerspace.name, ""+hackerspace.members,""+hackerspace.growthPerMonth);
					}
					/*
					for (int i = 0; i < 200; i++)
					{
		...
		public void Search()
		{
			// If there is search text, check if it has changed
			if (searchText.text != "" && searchText.text != previousSearchTextString)
			{
				previousSearchTextString = searchText.text;
				bool matchFound = false;
				List<ListViewItem> listViewItemsToRemove = new List<ListViewItem>();
				foreach ( ListViewItem lvi in this.ListView.Items )
				{
					Debug.Log("lvi.Name:"+lvi.Name);
					if (lvi!=null && lvi.Name != null && searchText.text!="")
					{
						if (lvi.Name.Contains(searchText.text))
						{
							matchFound = true;
						}
						else
						{
							listViewItemsToRemove.Add(lvi);
						}
					}
				}
				if (listViewItemsToRemove.Count>0)
				{
					foreach(ListViewItem lviToRemove in listViewItemsToRemove)
					{
						this.ListView.Items.Remove(lviToRemove);
					}
				}
			}
			else if (searchText.text == "" && searchText.text != previousSearchTextString)
			{
				previousSearchTextString = searchText.text;
				RefreshTestData();
			}
