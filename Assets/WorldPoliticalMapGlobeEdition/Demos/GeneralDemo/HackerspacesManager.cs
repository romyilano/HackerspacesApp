...
namespace Hackerspaces {
	...
	public class HackerspacesManager : MonoBehaviour {
		
		public static HackerspacesManager Instance;
		...
		public int mapCitySelectedIndex = 0;
		public bool isMapCitySelected = false;
		
		public bool isMapRegionSelected = false;
		
		public bool isMapCountrySelected = false;
		
		#endregion
		
		#region Net simulation
		
		public float lastTime = 0f;
		public float timeBetweenConnections = 0.1f;
		
		public void VisualizeNet(){
			if ( range == ZoomRange.Middle && Time.time - lastTime > timeBetweenConnections)
			{
				AddTrajectories ();
				lastTime = Time.time;
			}
...
//			map.FlyToCountry ("India");
		...
		/// <summary>
		/// If we click on a city, select it and show its hackerspaces.
		/// </summary>
		/// <param name="cityIndex"></param>
		public void OnCityClickHackerspaces (int cityIndex)
		{
			Debug.Log ("Clicked city " + cityIndex + " " + map.cities [cityIndex].name + " hackerspaces: 0");
			// If a city was selected already, deselect it
			if (isMapCitySelected)
			{
				DeselectCity();
			}
			// We're now selecting a city
			isMapCitySelected=true;
			mapCitySelectedIndex = cityIndex;
			map.ToggleCityHighlight(cityIndex,Color.white, true);
			ShowHackerspaces();
		}
		
		void StartHackerspaces () {
			listGO = GameObject.Find("Text City Hackerspaces List");
		}
		...
		/// <summary>
		/// Goes back to the previous selection, whatever it was
		/// </summary>
		public void Back()
		{
			// show history
		...
		/// <summary>
		/// Goes back to the next selection, whatever it was, unless it is most recent, in which case hide the forward button
		/// </summary>
		public void Forward()
		{
			// Show history
		}
		#endregion Selection History
		
		#region Selection Hierarchy
		/// <summary>
		/// Goes up a level to the parent entity
		/// </summary>
		public void Up()
		{
			if (isMapCitySelected)
			{
				// Select the province of the city
				SelectParentProvince();
				DeselectCity();
			}
			if (HackerspacesManager.Instance.map.provinceRegionHighlighted != null)
			{
				// Select the country of the province
			}
			if (HackerspacesManager.Instance.map.countryHighlighted != null)
			{
				// Select the planet of the country
			}
		}
		#endregion Selection Hierarchy
		
		#region Selection Province 
		
		public void SelectParentProvince ()
		{
			if(map.cityHighlighted != null)
			{
				string soughtProvinceName = map.cityHighlighted.province;
				SelectProvinceName(soughtProvinceName);
			}
		}
		
		public void SelectProvinceName(string provinceName)
		{
			int provinceIndex = 0;
			bool provinceWasFound = false;
			Province provinceFound = null;
			foreach ( Province province in map.provinces )
			{
				if (province.name == provinceName)
				{
					provinceWasFound=true;
			...
			if (provinceWasFound)
			{
				map.HighlightProvinceRegion(provinceIndex, provinceFound.mainRegionIndex,true);
					// show province stats like area
			}
		}
		#endregion Selection Province
		
		#region Selection City
		/// <summary>
		/// Called when selecting a new city or selecting something other than a city like a country
		/// </summary>
		/// <param name="cityIndex"></param>
		public void DeselectCity () {
			isMapCitySelected = false;
			map.ToggleCityHighlight(mapCitySelectedIndex,Color.blue, false);
			HideHackerspaces();
		}
		
		#endregion Selection City
		
		#endregion Selection
		
		public GameObject listGO;
		
		public void ShowHackerspaces()
		{
			listGO.SetActive(true);
			Text listText = listGO.GetComponent<Text>();
			List<Simulator.Hackerspace> spaces = Simulator.Instance.HackerspacesInCityIndex(mapCitySelectedIndex);
			int numberOfSpaces = spaces.Count;
			listText.text = map.cities[mapCitySelectedIndex].name + "(" + numberOfSpaces + ")";
			foreach (Simulator.Hackerspace space in spaces)
			{
				listText.text+= "\n" + space.name + ": " + space.members;
			}
		}
		
		void HideHackerspaces()
		{
			listGO.SetActive(false);
		}
		
		public bool showOptionsGUI = true;
		...
		void OnGUI () {
			...
			if (showOptionsGUI)
			{
				...
				if (GUI.Button (new Rect (10, 170, 160, 30), "  Change Earth style", buttonStyle)) {
					map.earthStyle = (EARTH_STYLE)(((int)map.earthStyle + 1) % 8);
				}
				...
				if (GUI.Button (new Rect (10, 395, 180, 30), "  Fly to Oakland (City)", buttonStyle)) {
					FlyToCity ("Oakland");
				}
			...
			if (zoomLevel != prevZoomLevel) {
				...
				else
				{
					...
					map.showCursor = false;
					map.cityIconSize = 0.2f;
					map.autoRotationSpeed = 0.05f;
				}
			} // end if zoom level != previousZoomLevel
		}
		...
		public ZoomRange range = ZoomRange.Far;
...
//			km = 500;
		...
		void AddTrajectories () {
			...
			for (int line=0; line<5; line++) {
				...
				Color color = new Color( 1f,1f,0.95f,0.5f);
				float elevation = Random.Range (0, 0.5f); 	// elevation is % relative to the Earth radius
				float drawingDuration = Random.Range(0.1f,4f);
