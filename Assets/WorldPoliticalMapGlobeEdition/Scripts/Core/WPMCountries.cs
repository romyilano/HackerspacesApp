...
//			countries = new Country[countryCount]; 
	...
	#region Drawing stuff
		...
		void DrawMapLabels () { 
			...
			for (int countryIndex=0; countryIndex<countries.Length; countryIndex++) {
				...
				if (country.labelGameObject == null) {
					...
					Vector2 minMaxLat = country.regions[country.mainRegionIndex].minMaxLat;
					Vector2 minMaxLon = country.regions[country.mainRegionIndex].minMaxLon;
					PolygonPoint ptMax = new PolygonPoint(minMaxLat.y, minMaxLon.y);
					PolygonPoint ptMin = new PolygonPoint(minMaxLat.x, minMaxLon.x);
					country.labelSphereRectMax = GetSpherePointFromLatLon(ptMax);
					country.labelSphereRectMin = GetSpherePointFromLatLon(ptMin);
		...
		void FadeCountryLabels() {
			...
			const float labelFadeMinFallOff = 0.1f;
			const float labelFadeMaxFallOff = 0.5f;
			for (int k=0;k<countries.Length;k++) {
				Country country = countries[k];
				...
				if (tm!=null) {
					...
					if (_countryLabelsEnableAutomaticFade)  {
						float y0 = Camera.main.WorldToViewportPoint(transform.TransformPoint(country.labelSphereRectMax)).y;
						float y1 = Camera.main.WorldToViewportPoint(transform.TransformPoint(country.labelSphereRectMin)).y;
						float screenHeight = Math.Abs ( y0 - y1);
						if (screenHeight<_countryLabelsAutoFadeMinHeight) {
							ad = Mathf.Lerp(1.0f, 0, (_countryLabelsAutoFadeMinHeight - screenHeight) / labelFadeMinFallOff);
						} else if (screenHeight>_countryLabelsAutoFadeMaxHeight) {
							ad = Mathf.Lerp (1.0f, 0, (screenHeight - _countryLabelsAutoFadeMaxHeight) / labelFadeMaxFallOff);
						}
					}
