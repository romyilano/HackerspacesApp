...
namespace WPM {
	...
	public partial class WorldMapGlobe : MonoBehaviour {
		...
		bool leftMouseButtonClick, rightMouseButtonClick;
	...
	#region System initialization
		...
		void Update () {
			...
			leftMouseButtonClick = Input.GetMouseButtonDown(0) || simulatedMouseButtonClick==1;
	...
	#region Highlighting
		...
		void CheckMousePos () {
			...
			if (_earthInvertedMode) {
				hits = Physics.RaycastAll (ray.origin + ray.direction * 5.0f, -ray.direction, 100, layerMask);
			} else {
				hits = Physics.RaycastAll (Camera.main.transform.position, ray.direction, 100, layerMask);
			}
			if (hits.Length > 0) {
				for (int k=0; k<hits.Length; k++) {
					if (hits [k].collider.gameObject == gameObject) {
						// Cursor follow
						if (_enableCountryHighlight || _dragConstantSpeed || (_cursorFollowMouse && _showCursor)) { // need the cursor location for highlighting test and constant drag speed
